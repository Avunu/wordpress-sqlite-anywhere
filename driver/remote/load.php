 ...
 *
 * With WP_TURSO_SNAPSHOT pointing at a readable file, reads are served from
 * that snapshot and writes go to the primary. A configured snapshot that is
 * not there yet means the publisher has not run: serving everything from the
 * primary is slower but correct, which is the right way to fail.
 *
 * @param  callable $config The configuration lookup.
 * @return WP_SQLite_Connection_Interface The connection.
...
function wp_sqlite_remote_create_turso_connection( callable $config ): WP_SQLite_Connection_Interface {
	...
	return wp_sqlite_turso_create_connection(
		$url,
		null === $token ? null : (string) $token,
		$snapshot,
		array(
