...
-   **`WP_TURSO_REPLICA`**: the [`wp_turso`](packages/php-ext-wp-turso/) PHP extension holds an embedded replica open in the PHP process and pulls the primary's changes into it every `WP_TURSO_REPLICA_PULL_MS`. Reads come from the replica at local-SQLite speed; writes go to the primary, and the replica pulls before the next read that follows a write, so a request reads its own writes while still reading locally — one pull costs about one primary round trip, where latching the rest of the request to the primary would cost one per statement (a plugin that writes a visitor session on every page view would otherwise send the whole page over the WAN; on a real site that was 10 s against 2.5 s). A request that wrote also pulls the replica up to date before it ends, so the next request — the redirect after a save, the dashboard after a login — finds what was written. Transactions still latch to the primary. No publisher, no snapshot copy, and it works for wp-admin too: on a WAN primary the dashboard went from 1.5 s to 40 ms. One process owns the replica file (an exclusive lock), so this is for FrankenPHP's one-process-many-threads model, not a pool of PHP-FPM workers; a fleet of front ends behind one balancer each keep their own replica.
